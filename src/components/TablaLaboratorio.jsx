...
function TablaLaboratorio({ refreshKey }) {
  ...
  async function fetchRegistros() {
    ...
    if (!error && data) {
      const sorted = [...data].sort((a, b) => {
        const aNeed = a.transferencia == null ? 0 : 1
        const bNeed = b.transferencia == null ? 0 : 1
  ...
  async function handleSave(id, record) {
    if (!editValue) return
    setSavingId(id)
    const updates = { transferencia: parseFloat(editValue) }
    if (record.recepcion != null) {
  ...
  const pendientes = registros.filter(r => r.transferencia == null)
        ...
        <>
          ...
          <div className="table-wrapper desktop-only">
            <table id="tabla-laboratorio">
              ...
              <tbody>
                {registros.map((r) => {
                  const needsMine = r.transferencia == null
                  const isEditing = editingId === r.id
                  const isMine = r.creado_por_rol === 'laboratorio'
                  ...
                  return (
                    <tr key={r.id} className={`${r.estado === 'completado' ? 'row-completed' : ''} ${needsMine ? 'row-needs-input' : ''}`}>
                      <td className="td-producto">
                        ...
                        {!isMine && needsMine && <span className="tag-from" style={{ marginTop: '0.25rem' }}>de Mikele</span>}
                      </td>
                      <td className="td-peso">
                        {isEditing ? (
                          ...
                          <span className="peso-value">{Number(r.transferencia).toLocaleString('es-MX')} g</span>
                        )}
                      </td>
                      ...
                      <td className="td-actions">
                        {!isEditing && !needsMine && (
                          <button className="btn-icon btn-edit" onClick={() => { setEditingId(r.id); setEditValue(r.transferencia || '') }} title="Editar">✏️</button>
                        )}
                      </td>
                    </tr>
                  )
                ...
                <div key={r.id} className={`record-card ${needsMine ? 'card-needs-input' : ''} ${r.estado === 'completado' ? 'card-completed' : ''}`}>
                  ...
                  <div className="record-card-body">
                    <div className="record-card-field">
                      <span className="record-card-label">Peso Transferencia</span>
                      {isEditing ? (
                        ...
                        <div className="record-card-value-row">
                          <span className="peso-value">{Number(r.transferencia).toLocaleString('es-MX')} g</span>
                          <button className="btn-icon btn-edit" onClick={() => { setEditingId(r.id); setEditValue(r.transferencia || '') }}>✏️</button>
                        </div>
                      )}
                    </div>
                    <div className="record-card-meta">
                      <span className="record-card-date">{formatDate(r.created_at)}</span>
                      {!isMine && needsMine && <span className="tag-from">de Mikele</span>}
                    </div>
                  </div>
                </div>
              )
